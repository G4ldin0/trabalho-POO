...
public class RegistroPacientesController implements Initializable {
    ...
    public void buscar() {
    	...
    	if(dto.getCpf().equals("") || dto.getCpf().equals(" ")) {
    		List<PacienteDTO> todos = bo.listar();
        	listaPacientesFiltrados = FXCollections.observableArrayList(todos);
    	}
    	else {
    		List<PacienteDTO> pacCPF = bo.listarPorCpf(dto);
    		List<PacienteDTO> pacNome = bo.listarPorNome(dto);
    		List<PacienteDTO> pac = new ArrayList<PacienteDTO>();
    		for (int i = 0; i < pacCPF.size(); i++) {
    			pac.add(pacCPF.get(i));
    		}
    		for (int i = 0; i < pacNome.size(); i++) {
    			pac.add(pacNome.get(i));
    		}
        	listaPacientesFiltrados = FXCollections.observableArrayList(pac);
    	}
    	columnNome.setCellValueFactory(new PropertyValueFactory<>("nome"));
        columnCpf.setCellValueFactory(new PropertyValueFactory<>("cpf"));
        columnEndereco.setCellValueFactory(new PropertyValueFactory<>("endereco"));
