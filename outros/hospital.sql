...
CREATE TABLE usuario (username VARCHAR(30), senha VARCHAR(30));
CREATE TABLE adm (username VARCHAR(30), senha VARCHAR(30));
...
CREATE TABLE Consulta(dia DATE, horario TIME,idPaciente INT NOT NULL, idMedico INT NOT NULL, idProntuario INT NOT NULL, idConsulta INT NOT NULL AUTO_INCREMENT,PRIMARY KEY(idConsulta), FOREIGN KEY(idPaciente) REFERENCES Paciente(idPaciente),FOREIGN KEY(idProntuario) REFERENCES Prontuario(idProntuario),FOREIGN KEY(idMedico) REFERENCES Medico(idMedico));
